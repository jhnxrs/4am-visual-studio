...
export const GetInTouchV2 = (props: Props) => {
    ...
    useEffect(() => {
        const calculateRange = () => {
            ...
            const maxScrollPx = Math.max(doc.scrollHeight - viewportHeight, 0);
    ...
    useEffect(() => {
        ...
        const step =
            wordEls.length > 1 ? maxWordStart / (wordEls.length - 1) : 0;
        ...
        const contactStart = 0.72;
        const contactProgress = clamp(
            (baseProgress - contactStart) / contactDuration,
            0,
            1
        );
    ...
    return (
        <section
            id="get-in-touch"
            className="w-screen pt-32 pb-48 bg-[#eee] px-12 relative z-20"
        >
            <div
                ref={wordsRef}
            ...
            >
                ...
                >
                    05<span className="text-black/40">{"//"}</span>
                    {t("sectionTitle")}
                </p>
